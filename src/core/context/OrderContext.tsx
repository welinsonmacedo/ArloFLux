...
type OrderAction = 
  ...
  | { type: 'UNLOCK_AUDIO' }
  | { type: 'SET_STATE'; state: Partial<OrderState> }
 ...
 useEffect(() => {
  ...
  useEffect(() => {
      ...
      const channel = supabase.channel(`restaurant_room:${tenantId}`)
          .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'orders', filter: `tenant_id=eq.${tenantId}` }, handleRealtimeUpdate)
          .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'orders', filter: `tenant_id=eq.${tenantId}` }, handleRealtimeUpdate)
          .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'order_items', filter: `tenant_id=eq.${tenantId}` }, handleRealtimeUpdate)
          .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'order_items', filter: `tenant_id=eq.${tenantId}` }, handleRealtimeUpdate)
          .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'service_calls', filter: `tenant_id=eq.${tenantId}` }, handleRealtimeUpdate)
          .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'service_calls', filter: `tenant_id=eq.${tenantId}` }, handleRealtimeUpdate)
          .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'restaurant_tables', filter: `tenant_id=eq.${tenantId}` }, handleRealtimeUpdate)
  ...
  const placeOrder = async (params: PlaceOrderParams) => {
      ...
      if (!navigator.onLine) {
          ...
          showAlert({ title: "Modo Offline", message: "Pedido salvo localmente e será sincronizado ao restaurar a conexão.", type: "SUCCESS" });
          fetchData(true);
          return;
      }
      ...
      if (error) {
          ...
          if (authState.currentUser) {
              await logAudit(tenantId, authState.currentUser.id, authState.currentUser.name, 'ORDERS', 'Criação de Pedido', { orderType: finalOrderType, tableId });
          }
          fetchData();
      }
  };
  ...
  const cancelOrder = async (orderId: string) => {
      ...
      if (!navigator.onLine) {
          ...
          showAlert({ title: "Modo Offline", message: "Cancelamento salvo localmente.", type: "SUCCESS" });
          fetchData(true);
          return;
      }
      ...
      if (authState.currentUser) {
          await logAudit(tenantId, authState.currentUser.id, authState.currentUser.name, 'ORDERS', 'Cancelamento de Pedido', { orderId });
      }
      fetchData();
  };
      ...
      if (!navigator.onLine) {
          await offlineService.addPendingOperation({
              ...
              data: {
                  ...
                  method: safeData.method,
                  items: enrichedItems,
                  cashierName: safeData.cashierName || 'Sistema'
              }
          });
          fetchData(true);
          return;
  ...
  const dispatchOrder = async (orderId: string, courierInfo: { id: string, name: string }) => {
      ...
      if (!navigator.onLine) {
          ...
          showAlert({ title: "Modo Offline", message: "Despacho salvo localmente.", type: "SUCCESS" });
          fetchData(true);
          return;
      }
      ...
      if (!navigator.onLine) {
          await offlineService.addPendingOperation({
              ...
              data: {
                  ...
                  specificOrderIds: specificOrderIds || null,
                  courierInfo: courierInfo || null
              }
          });
          showAlert({ title: "Modo Offline", message: "Pagamento salvo localmente.", type: "SUCCESS" });
          fetchData(true);
          return;
      }
  ...
  const addTable = async () => {
      ...
      if (!navigator.onLine) {
          ...
          showAlert({ title: "Modo Offline", message: "Mesa será adicionada ao restaurar a conexão.", type: "SUCCESS" });
          return;
      }
          ...
          case 'DELETE_TABLE': 
              if (!navigator.onLine) {
                  await offlineService.addPendingOperation({
                      ...
                      data: { id: action.tableId },
                      tenantId: tenantId || ''
                  });
                  fetchData(true);
                  return;
              }
              await supabase.from('restaurant_tables').delete().eq('id', action.tableId).eq('tenant_id', tenantId); 
              fetchData(); 
              break;
           case 'OPEN_TABLE': 
              ...
              if (!navigator.onLine) {
                  ...
                  showAlert({ title: "Modo Offline", message: "Abertura de mesa salva localmente.", type: "SUCCESS" });
                  fetchData(true);
                  return;
              }
  ...
  return (
    <OrderContext.Provider value={{ 
        ...
        fetchData: async () => { await fetchData(true); }
    }}>
      {children}
