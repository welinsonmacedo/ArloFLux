...
export const MenuProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const fetchProducts = useCallback(async () => {
      ...
      const cached = localStorage.getItem(`menu_cache_${tenantId}`);
      if (cached) {
          try {
              const parsed = JSON.parse(cached);
              setState({ products: parsed, isLoading: false });
          } catch (e) {
              console.error("Erro ao carregar cache do menu:", e);
          }
      }
      ...
      try {
          const { data, error } = await supabase.from('products').select('*').eq('tenant_id', tenantId).is('deleted_at', null);
          if (error) {
              console.error('MenuContext: Error fetching products:', error);
          }
          const mapped = data ? mapProducts(data) : [];
          setState({ products: mapped, isLoading: false });
          
          // Update cache
          localStorage.setItem(`menu_cache_${tenantId}`, JSON.stringify(mapped));
      } catch (err) {
  ...
  useEffect(() => {
      if (tenantId) {
          ...
          const channel = supabase.channel(`menu:${tenantId}`)
              .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'products', filter: `tenant_id=eq.${tenantId}` }, fetchProducts)
              .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'products', filter: `tenant_id=eq.${tenantId}` }, fetchProducts)
              .subscribe();
