...
- **Relatórios (`AdminReports.tsx`):** Extração de dados de vendas, produtos mais vendidos e comissões. Utiliza *Materialized Views* no banco de dados para relatórios de alta performance (vendas diárias, mensais e top produtos).
...
- **Produção & Fichas Técnicas (`InventoryProductionView.tsx`):** Criação de receitas (fichas técnicas) que dão baixa automática nos insumos quando um prato é vendido. Gatilhos de banco de dados (Triggers) garantem a integridade da baixa de estoque de forma segura.
...
- **Cozinha / KDS (`KitchenDisplay.tsx`):** Tela de exibição de pedidos para a cozinha, com controle de tempo de preparo e status (Pendente, Preparando, Pronto). Utiliza o hook `useRealtimeKDS` com inscrições direcionadas no banco de dados para atualizações em tempo real otimizadas.
- **Ponto Eletrônico (`TimeClock.tsx`):** Interface simples (tablet/totem) para os funcionários baterem o ponto usando PIN ou QR Code.
- **Gestão de Mesas (`src/modules/tables`):** Módulo dedicado para gerenciamento do salão, com o componente `TableCard` e o hook `useTables` para atualizações em tempo real do status das mesas.
...
- Histórico de pedidos do cliente (`ClientHistory.tsx`), com suporte a exclusão lógica (Soft Delete) para manter a integridade dos dados históricos.
...
- **Feature Flags (`tenant_features`):** Controle granular de funcionalidades ativas por inquilino, permitindo lançamentos graduais e testes A/B.
...
- **Auditoria (`AuditDashboard.tsx`):** Registro imutável de todas as ações críticas realizadas no sistema (quem apagou um pedido, quem alterou um preço, etc.) para fins de segurança e compliance. Os logs de auditoria são enriquecidos com metadados (User-Agent, URL) e ambiente de execução.

---

## Resumo do Fluxo de Dados (Contextos)
O sistema utiliza a Context API do React para gerenciar o estado global, com atualizações em tempo real (Realtime) otimizadas via Supabase:
...
- `OrderContext.tsx`: Gerencia o ciclo de vida dos pedidos (criação, preparo, pagamento). Utiliza Soft Delete para cancelamentos.
...
- `MenuContext.tsx`: Gerencia o catálogo de produtos e categorias.

*Nota: O sistema conta com índices de banco de dados otimizados para garantir alta performance em consultas frequentes, como busca de pedidos por status e logs de auditoria.*
